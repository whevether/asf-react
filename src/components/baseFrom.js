...
import { Form, Button, Input, Select,notification, Cascader } from 'antd';
const BaseFrom = (props) => {
  ...
  const renderFromItem = (item) => {
    if(item?.fromType === 'select'){
      ...
      return <Input placeholder={item?.placeholder} allowClear={item?.options?.allowClear} />;
    }
  };
  return(
    <Form layout={props?.layout ?? 'vertical'} className="base-from" ref={fromRef} name="basefrom" onFinish={props?.onFinish} onFinishFailed={onFinishFailed}>
       {
