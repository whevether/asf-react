...
const BaseFrom = (props) => {
  ...
  const renderFromItem = (item, i) => {
    if (item?.fromType === 'select') {
      ...
      return <Form.Item name={item?.name} label={item?.title} key={i} rules={item?.rules}><Cascader placeholder={item?.placeholder} allowClear={item?.options?.allowClear} options={item?.selOption} /> </Form.Item>;
